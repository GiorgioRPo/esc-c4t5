...
- Instant search over ~170 curated, well-known destinations, plus a lazily
  ...
  once it finishes loading (no impact on initial page load — it's a separate
  code-split chunk)
...
- Destination recommendations — related destinations shown alongside search
  results, powered by the recommendation engine (see below)
...
- `POST /api/bookings` creates a Stripe PaymentIntent; the booking row is
  written by a Stripe webhook once payment succeeds
...
- Separate FastAPI service that ranks related destinations using semantic
  (embedding) + geographic candidate retrieval over a curated destination set
...
running the project. Quickstart below assumes you already have the `.env`
...
See [`docs/SETUP.md`](docs/SETUP.md) §6 for the one-time destination-seeding step.
